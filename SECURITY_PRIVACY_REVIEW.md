...
- 🔴 **Wardrobe photos live at public URLs.** `src/lib/api.ts` uses `getPublicUrl`, which only works if the `wardrobe-images` bucket is public — so every item photo and outfit selfie is viewable by anyone holding the URL, with no auth. Paths contain unguessable UUIDs (user id + random file id), so enumeration isn't practical, but URLs leak: shared screenshots, browser history, CDN caches, referrer headers. **Fix:** switch the bucket to private and replace `getPublicUrl` with `createSignedUrl` (e.g. 7-day expiry) — a contained refactor in `api.ts` plus re-resolving URLs on load. Or consciously accept UUID-obscurity for now; decide, don't default.
...
- 🔴 **No consent or privacy surface anywhere.** Signup collects name, gender, city, style preferences, spending signals, and photos — and sends wardrobe + preferences + photos to the Anthropic API — with no privacy note, policy page, or disclosure. (Anthropic's API doesn't train on your data by default and has limited retention, which is worth *saying*.) **Fix:** one static privacy page + a single line on the login screen ("By continuing you agree… Photos and wardrobe data are processed by Anthropic's Claude to power suggestions"). Required before Google OAuth app verification or any store listing anyway.
