...
serve(async (req) => {
  ...
  try {
    ...
    const vibeAllowsAnchor = (i: WardrobeItem) =>
    ...
    if (pinned_item_ids?.length) {
      ...
      const pool = filtered.filter(i => anchorCategories.has(i.category) && vibeAllowsAnchor(i))
      if (pool.length > 1) {
    ...
    if (excludeSet.size > 0) {
      ...
      const notPrevious = cleanItems.filter(i => !excludeSet.has(i.id) && anchorCategories.has(i.category) && vibeAllowsAnchor(i))
